...
export const PILL =
	'text-xs bg-neutral-100 border border-default rounded-full px-2 py-0.5 text-subtle inline-block w-fit cursor-pointer hover:bg-neutral-200 transition-colors color-swatch color-swatch-no-hover';

/** Copyable token pill used in component demos — same look, with swatch hover. */
export const PILL_SWATCH =
	'color-swatch text-xs bg-elevation-surface-default border border-default rounded-full px-2 py-0.5 text-subtle cursor-pointer inline-block';
	...
	return `
		<div class="flex gap-4 items-stretch">
			<div class="flex-1 rounded-lg overflow-hidden flex flex-col">
				<div class="bg-neutral-100 p-6 flex items-center justify-center flex-1">${doExample}</div>
				<div class="border-t-[3px] border-success"></div>
				<div class="bg-success-default p-4 flex-1">
			...
			<div class="flex-1 rounded-lg overflow-hidden flex flex-col">
				<div class="bg-neutral-100 p-6 flex items-center justify-center flex-1">${dontExample}</div>
				<div class="border-t-[3px] border-danger"></div>
				<div class="bg-danger-default p-4 flex-1">
					<div class="flex items-center gap-1.5 mb-1">
