...
export type CapacityIndicatorProps = {
	...
	context?: BCC_CONTEXT;
	/** Icon component shown instead of the numeric value. */
	icon?: VueComponent;
	/** When true, hides the outline/progress ring and shows a filled background instead. */
	borderless?: boolean;
};

const props = withDefaults(defineProps<CapacityIndicatorProps>(), {
	used: 0,
	size: 'base',
...
const { value: valueUsed } = useAnimatedNumber(toRef(props, 'used'), props.used, props.animationDuration);
...
<template>
	<svg
		...
		:class="[
			...
			{ colored, borderless, 'is-full': progress >= 100, 'is-warning': progress >= 50 && progress < 100 },
			context ? `ctx-${context}` : '',
		]"
	>
		<template v-if="squared">
			<rect width="36" height="36" x="2" y="2" rx="4" fill="none" stroke-width="4" class="circle" />
			<path
				v-if="progress > 0"
		...
		<template v-else>
			<circle cx="20" cy="20" r="18" fill="none" stroke-width="3" stroke="currentColor" class="circle" />
			<circle
				v-if="progress > 0"
				cx="20"
				cy="20"
