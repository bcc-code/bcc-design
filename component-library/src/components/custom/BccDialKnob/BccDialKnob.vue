...
const backgroundArcAngle = computed(() =>
	Math.max(-359.99, Math.min(359.99, Math.max(minDegrees.value, Math.min(totalAngle.value, maxDegrees.value))))
);
...
function drawCanvas() {
	...
	if (Math.abs(backgroundArcAngle.value) >= 108) {
		...
		ctx.arc(center.value, center.value, radius.value, toRad(0), toRad(backgroundArcAngle.value), anticlockwise);
		if (props.colored) {
...
function onDrag(e: MouseEvent | TouchEvent) {
	...
	animationFrame = requestAnimationFrame(() => {
		...
		if (delta < -180) delta += 360;
		const newTotalAngle = Math.round(Math.max(minDegrees.value, Math.min(maxDegrees.value, totalAngle.value + delta)));
...
// Listen for dark mode changes and reload colors.
