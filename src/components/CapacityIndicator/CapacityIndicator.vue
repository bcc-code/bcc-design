...
const classes = computed(() => {
  return {
    outerCircle: {
      "stroke-neutral-300": progress.value < 50,
      "stroke-muddy-waters-100": progress.value >= 50 && progress.value < 100,
      "stroke-red-800": progress.value >= 100,
    },
    progressCircle: {
      "stroke-neutral-500": progress.value < 50,
      "stroke-muddy-waters-400": progress.value >= 50 && progress.value < 100,
      "stroke-red-800": progress.value >= 100,
    },
    text: {
      "text-neutral-900": progress.value < 50,
      "text-muddy-waters-600": progress.value >= 50 && progress.value < 100,
      "text-red-800": progress.value >= 100,
    },
  };
});
...
<template>
  <svg width="2.5em" height="2.5em" viewBox="0 0 40 40">
    <circle cx="20" cy="20" r="18" fill="none" stroke-width="2" :class="classes.outerCircle" />
    <circle
      v-if="progress > -1"
      cx="20"
      cy="20"
